...
export function ScannerMesh({ chipPresent, writing = false }: ScannerMeshProps) {
  ...
  useFrame((_, delta) => {
    const target = writing ? 1.35 : chipPresent ? 1 : 0.3;
    glow.current += (target - glow.current) * Math.min(1, delta * 4.2);
    const speed = writing ? 4.2 : chipPresent ? 2.4 : 1;
    ...
    if (shell.current) {
      ...
      mat.opacity = 0.25 + g * 0.28;
      shell.current.scale.setScalar(writing ? 1.04 : 1);
    }
    ...
    if (chip.current) {
      ...
      chip.current.scale.setScalar(writing ? 1.06 : 1);
    }

    if (core.current) {
      const mat = core.current.material as MeshBasicMaterial;
      mat.color.setHex(writing ? AMBER : ACC_GLOW);
      mat.opacity = g * (writing ? 0.72 : 0.5);
      const pulse = writing
        ? 1.12 + Math.sin(t * 0.012) * 0.1
